 ...
 */
static UNUSED word_t convertAccessToArch(word_t access)
{
    switch (access) {
...
getBreakpoint_t getBreakpoint(tcb_t *t, uint16_t bp_num)
{
    ...
    if (ret.type == seL4_InstructionBreakpoint) {
        ...
        bcr.words[0] = readBcrContext(t, bp_num);
        /* @alwin: Don't think this is a thing on aarch64 */
...
BOOT_CODE bool_t Arch_initHardwareBreakpoints(void)
{
    ...
    // @alwin: why does the below not work? if anything the top one should not work
//    word_t oslar = 0;
//    MRS("oslar_el1", oslar);
//    oslar &= ~OSLAR_LOCK;
//    MSR("oslar_el1", oslar);
...
//            userError("%lx %lx", readBvrContext(t, i), readBcrContext(t, i));
